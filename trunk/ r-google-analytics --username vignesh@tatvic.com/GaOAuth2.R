...
GenerateAccessToken <- function(){
	...
	if (!file.exists(file.path(system.file(package = "RGoogleAnalytics"), 
	  ...
	  # For building request query for 
	  ...
	  # For retriving the refresh token
	  ...
	  save(accesstoken,client_id,client_secret,
						file = file.path(system.file(package = "RGoogleAnalytics"),
						"accesstoken.rda"),envir = .GlobalEnv)                                            
	  ...
	  attach(file.path(system.file(package = "RGoogleAnalytics"), 
					   "accesstoken.rda"),warn.conflicts = F)
...
# extract the Google Analytics data from another Google Account    
...
RefreshToAccesstoken = function(accesstoken){
	rt = fromJSON(postForm('https://accounts.google.com/o/oauth2/token',
			refresh_token = accesstoken$refresh_token, client_id = client_id,
