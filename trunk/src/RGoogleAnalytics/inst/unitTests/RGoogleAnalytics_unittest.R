...
# Test the parsing of JSON feed after it has been returned for profile
# information.
...
TestParseDataFeedJSON <- function(){
  ...
  DataFeed.json.totalsForAllResults.visits <- 
    DataFeed.json$totalsForAllResults$`ga:visits`
  DataFeed.json.totalsForAllResults.bounces <- 
    DataFeed.json$totalsForAllResults$`ga:bounces`
  ...
  json.aggr.totals <- 
    as.data.frame(rbind(as.numeric(DataFeed.json.totalsForAllResults.visits), 
